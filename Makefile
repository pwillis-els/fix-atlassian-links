...
search-replace-page: docker-build
	docker run --rm -it fix-atlassian-links --page_id $(CONFLUENCE_PAGE_ID) --search "$(SEARCH_RE)" --replace "$(REPLACE_TEXT)" --username "$(CONFLUENCE_USER)" --url "$(CONFLUENCE_URL)" --auth_token "$(CONFLUENCE_PASS)" search-replace-page
